...
//			indent = 0;
		...
		{
			index = System.Math.Min( data.Length, index + step );
			return data[ index ];
		}
		
		private char StepBackward( int step = 1 )
		{
			index = System.Math.Max( 0, index - step );
			return data[ index ];
		}
...
//		}
		...
		private object ParseEntity()
		{
			...
			if( word.Length != 24 && Regex.IsMatch( word, @"^\d+$" ) ) {
				return System.Int32.Parse( word );
			}
		...
		private bool SerializeValue( object value, StringBuilder builder, bool readable = false )
		{
			if( value == null ) {
				...
				SerializeString( System.Convert.ToString( (char)value ), builder, readable );
			}
			else if( value is bool ) {
				builder.Append( System.Convert.ToInt32( value ).ToString() );
			}
			else if( value.GetType().IsPrimitive ) {
				builder.Append( System.Convert.ToString( value ) );
			}
