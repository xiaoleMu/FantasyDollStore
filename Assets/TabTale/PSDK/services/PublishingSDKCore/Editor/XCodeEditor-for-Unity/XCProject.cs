...
namespace PSDK.UnityEditor.XCodeEditor
{
...
//		private string _filePath;
		...
		public XCProject( string filePath ) : this()
		{
			if( !System.IO.Directory.Exists( filePath ) ) {
				Debug.LogWarning( "Path does not exists:" + filePath );
				return;
			}
...
//				_rootGroup = (PBXDictionary)_objects[ (string)_rootObject[ "mainGroup" ] ];
		...
		PBXDictionary<PBXVariantGroup> _variantGroup;
		public PBXDictionary<PBXVariantGroup> variantGroups {
			get {
				if( _variantGroup == null ) {
					_variantGroup = new PBXDictionary<PBXVariantGroup>( _objects );
				}
		...
		public bool SetDebugInformationFormat (string value)
		{
			foreach( KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations ) {
				buildConfig.Value.SetDebugInformationFormat( value );
			}
			modified = true;
			return modified;
		}
...
//		#endregion
		...
		public void ApplyMod( string pbxmod , string dataPath = null)
		{
			XCMod mod = new XCMod( pbxmod , dataPath);
			ApplyMod( mod );
		}
		...
		{	
			...
			this.SetDebugInformationFormat("dwarf");
