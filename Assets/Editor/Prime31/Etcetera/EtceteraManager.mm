...
@property (nonatomic) NSInteger maxImagesFromPicker;
@end
...
{
	self.maxImagesFromPicker = maxNumberOfPhotos;
...
- (void)processImageFromImagePicker:(UIImage*)image
{
	@autoreleasepool
	{
		BOOL wantsPngFile = self.JPEGCompression <= 0;
		...
		// Get a filepath pointing to the docs directory
		NSArray *dirs = NSSearchPathForDirectoriesInDomains( NSDocumentDirectory, NSUserDomainMask, YES );
		NSString *filename = [NSString stringWithFormat:@"%@.%@", [EtceteraManager stringWithNewUUID], wantsPngFile ? @"png" : @"jpg"];
		NSString *filePath = [[dirs objectAtIndex:0] stringByAppendingPathComponent:filename];
		...
		if( wantsPngFile )
			[UIImagePNGRepresentation( [image imageWithImageDataMatchingOrientation] ) writeToFile:filePath atomically:NO];
		else
			[UIImageJPEGRepresentation( [image imageWithImageDataMatchingOrientation], self.JPEGCompression ) writeToFile:filePath atomically:NO];
...
- (BOOL)assetsPickerController:(CTAssetsPickerController *)picker shouldSelectAsset:(PHAsset *)asset
{
	...
	return picker.selectedAssets.count < self.maxImagesFromPicker;
}
...
- (void)assetsPickerController:(CTAssetsPickerController*)picker didFinishPickingAssets:(NSArray*)assets
{
	...
	{
		CGSize targetSize = CGSizeMake( asset.pixelWidth * self.scaledImageSize, asset.pixelHeight * self.scaledImageSize );
