...
{
	NSString* filePath = GetStringParam( path );
	if( ![NSFileManager.defaultManager fileExistsAtPath:filePath] )
	{
		NSError* error = [NSError errorWithDomain:@"com.prime31.etcetera" code:666 userInfo:@{ NSLocalizedDescriptionKey: @"Invalid path to video file" }];
		[EtceteraManager.sharedManager video:filePath didFinishSavingWithError:error contextInfo:NULL];
		return;
	}
	...
	if( !UIVideoAtPathIsCompatibleWithSavedPhotosAlbum( filePath ) )
	{
		...
		// sometimes, some videos can be stuck in the PHPhotoLibrary but this isnt 100% a solution and fails when the video is not compatible so we wont make this live out of the box
...
//			 else
//				 NSLog( @"video failed to save. Error: %@", error );
//		 }];
		return;
	}
	
	UISaveVideoAtPathToSavedPhotosAlbum( filePath, EtceteraManager.sharedManager, @selector(video:didFinishSavingWithError:contextInfo:), NULL );
}
